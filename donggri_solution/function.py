...
def image_resize(img_url:str):
    ...
    src2=cv2.resize(src2, (800,270)) #글씨 작게 수정
...
def diary_ocr_main(image_url:str):
    w_list=[]
    image_path = image_url
    output = kakao_ocr(image_path).json()
    ...
    corr_spell = diary_check_spell(w_list)

    print(w_list)
    print(corr_spell)
    return corr_spell
    
def diary_check_spell(ocr_spell):
    w_list=[]
    for i in ocr_spell:
...
if __name__ == "__main__":
   ...
   # else:
   #    print('NO')
   kakao_ocr_init("C:\\donggri_solution\\static\\sorce\\Paint.jpg")
